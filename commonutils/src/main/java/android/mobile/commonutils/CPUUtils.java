...
public class CPUUtils {
     ...
     * Print cpu info.
     */
    public static String printCpuInfo(String charsetName) {
        String info = FileUtils.readFile(CPU_INFO_PATH, charsetName).toString();
        return info;
    }
     ...
     */
    public static long getCurrentFreqency(String charsetName) {
        try {
            return Long.parseLong(FileUtils.readFile(CPU_FREQ_CUR_PATH, charsetName).toString().trim());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
